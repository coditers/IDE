...
class ExecC extends Exec {
  ...
  ExecC(SourceCodeVo sourceCodeVo, ProblemVo problemVo){
    ...
    this.compileCommand = new String[] {"/usr/bin/gcc", "-std=gnu99", "-o", WORKSPACE_PATH+"sourcecode/"+sourceCodeVo.getId()+"/task", WORKSPACE_PATH+"sourcecode/" + sourceCodeVo.getId() + "/main.c"};
